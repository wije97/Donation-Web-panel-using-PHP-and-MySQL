<?php
$connect = new PDO("mysql:host=localhost;dbname=dbdonate;charset=utf8mb4", "root", "");
...
{
	$query = "SELECT fname FROM tblrequester WHERE nic = '$user_id'";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
	...
	if ($result != null) {
		...
		$query = "SELECT fname FROM tbldonor WHERE nic = '$user_id'";
		$statement = $connect->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
		foreach($result as $row)
